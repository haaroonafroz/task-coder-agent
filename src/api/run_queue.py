...
class RunQueue:
    ...
    def _execute(
        ...
        try:
            # Reload the session context from disk in case it changed since
            # enqueue (e.g. the caller created the session then immediately
            # started a run before the disk meta was flushed).
            fresh = self._session_manager.load_session(ctx.session_id) or ctx
            # Point the runtime at the chosen model for this run.
            self._runtime.model = model
            result = self._runtime.run(rec.request, session=fresh)
            rec.result = {
            ...
            rec.status = result.status  # completed | partial | failed
            # Append an assistant summary message so the chat log stays coherent.
            summary = (
                f"Run finished - status: {result.status}, "
                f"milestones {result.milestones_passed}/{result.milestones_total} passed, "
                f"elapsed {result.total_elapsed_ms / 1000:.1f}s."
            )
            self._message_store.append(fresh, "assistant", summary, run_id=rec.run_id)
