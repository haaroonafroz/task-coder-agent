...
Wraps the OpenAI-compatible vLLM endpoint and captures:
...
The only difference between modes is which endpoint URL is passed in.

HuggingFace token is loaded from .env (HF_TOKEN) and injected into the
environment so that vLLM's model downloader picks it up automatically.
"""
...
# Propagate HF_TOKEN so the HuggingFace Hub downloader inside vLLM can
# authenticate when fetching gated Gemma 4 model weights.
...
# ---------------------------------------------------------------------------
# Endpoint registry – overridable via .env (VLLM_BASELINE_URL / VLLM_SPECULATIVE_URL)
# ---------------------------------------------------------------------------
ENDPOINTS: dict[str, str] = {
    "baseline": os.getenv("VLLM_BASELINE_URL", "http://localhost:8000/v1"),
    "speculative": os.getenv("VLLM_SPECULATIVE_URL", "http://localhost:8001/v1"),
}
